...
def live_matches():
    ...
    import re
    team_score_pattern = re.compile(r"([A-Z]+)([0-9]+[-0-9]*|\.{2,}[0-9]+|\.{2,})")

    for i in range(len(matches)):
        text = matches[i].text.strip()
        # Add space between team and score, e.g., BAN133 -> BAN 133
        def add_space(match):
            return f"{match.group(1)} {match.group(2)}"
        new_text = team_score_pattern.sub(add_space, text)
        live_matches.append(new_text)
